...
public class ActivitiesPortlet extends MVCPortlet {
	...
	public void render(RenderRequest request, RenderResponse response) throws PortletException, IOException {
		...
		if (portletPreferences.getValue("init", StringPool.BLANK).equals(StringPool.BLANK)){
			for (String tab : ACTIVITY_TABS){
				if (tab.equals("all"))
					portletPreferences.setValue(tab, "true");
				else
					portletPreferences.setValue(tab, "false");
			}
			portletPreferences.setValue("init", "true");
	...
	public void getMicroblogsComments(SocialActivitySet activitySet, ResourceRequest resourceRequest,
		...
		List<MicroblogsEntry> microblogsEntries = MicroblogsEntryLocalServiceUtil
				.getReceiverMicroblogsEntryMicroblogsEntries(MicroblogsEntryConstants.TYPE_REPLY,
	...
	public void repostMicroblogsEntry(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		MicroblogsEntryServiceUtil.addMicroblogsEntry(themeDisplay.getUserId(), microblogsEntry.getContent(),
				...
				microblogsEntry.getMicroblogsEntryId(), microblogsEntry.getSocialRelationType(), serviceContext);
	}
	...
	public void updateMBComment(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		try {
			...
			if (cmd.equals(Constants.DELETE)) {
				...
				mbMessage = MBMessageServiceUtil.addDiscussionMessage(groupId, className, classPK, className, classPK,
						themeDisplay.getUserId(), mbThread.getThreadId(), rootMBMessage.getMessageId(),
						StringPool.BLANK, body, serviceContext);
			}

			if (mbMessage != null) {
				jsonObject = getJSONObject(mbMessage.getMessageId(), mbMessage.getBody(), mbMessage.getModifiedDate(),
						mbMessage.getUserId(), mbMessage.getUserName(), themeDisplay);
			}
	...
	public void updateMicroblogsComment(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		try {
			...
			if (cmd.equals(Constants.DELETE)) {
				...
				if (cmd.equals(Constants.EDIT) && (microblogsEntryId > 0)) {
					...
					microblogsEntry = MicroblogsEntryServiceUtil.addMicroblogsEntry(themeDisplay.getUserId(), body,
							MicroblogsEntryConstants.TYPE_REPLY, currentMicroblogsEntry.getUserId(), classPK,
							currentMicroblogsEntry.getSocialRelationType(), serviceContext);
				}
			}
			...
			if (microblogsEntry != null) {
				jsonObject = getJSONObject(microblogsEntry.getMicroblogsEntryId(), microblogsEntry.getContent(),
						microblogsEntry.getModifiedDate(), microblogsEntry.getUserId(), microblogsEntry.getUserName(),
						themeDisplay);
			}
	...
	protected JSONObject getJSONObject(long mbMessageIdOrMicroblogsEntryId, String body, Date modifiedDate,
		...
		jsonObject.put(
				"modifiedDate",
				Time.getRelativeTimeDescription(modifiedDate.getTime(), themeDisplay.getLocale(),
