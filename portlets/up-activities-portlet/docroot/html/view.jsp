...
<c:if test="<%= group.isUser() && layout.isPrivateLayout() && !oneTab%>">
	<liferay-ui:tabs
		names="<%= tabs%>"
...
<aui:script use="aui-base,aui-io-request-deprecated,aui-parse-content,liferay-so-scroll">
				...
				<portlet:renderURL var="viewActivitySetsURL" windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>">
					<c:choose>
						<c:when test="<%= GetterUtil.getBoolean(PropsUtil.get(PropsKeys.SOCIAL_ACTIVITY_SETS_ENABLED)) %>">
							<portlet:param name="mvcPath" value="/html/view_activity_sets.jsp" />
						</c:when>
						<c:otherwise>
							<portlet:param name="mvcPath" value="/html/view_activities.jsp" />
						</c:otherwise>
					</c:choose>
	...
	socialActivities.delegate(
		...
		function(event) {
			...
			var uri = '<portlet:renderURL windowState="<%= LiferayWindowState.POP_UP.toString() %>"><portlet:param name="mvcPath" value="/html/repost_microblogs_entry.jsp" /><portlet:param name="mvcPath" value="/html/repost_microblogs_entry.jsp" /><portlet:param name="redirect" value="<%= currentURL %>" /></portlet:renderURL>';
